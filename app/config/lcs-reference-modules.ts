...
export const lcsReferenceModules: FreightModule[] = [
  ...
  module({
    ...
    columns: [column('code', 'Code', 'លេខកូដ'), column('name', 'Name', 'ឈ្មោះ'), column('description', 'Description', 'បរិយាយ'), column('displayOrder', 'Display Order', 'លំដាប់បង្ហាញ'), column('status', 'Status', 'ស្ថានភាព')],
    fields: [field('code', 'Code', 'លេខកូដ', undefined, undefined, 'text', undefined, true), field('name', 'Name', 'ឈ្មោះ', undefined, undefined, 'text', undefined, true), field('description', 'Description', 'បរិយាយ', undefined, undefined, 'textarea', undefined, false, { colSpan: 2 }), field('displayOrder', 'Display Order', 'លំដាប់បង្ហាញ', 'Display', 'ការបង្ហាញ', 'number'), field('status', 'Status', 'ស្ថានភាព', 'Control', 'ការគ្រប់គ្រង', 'select', ACTIVE_STATUS)],
    filters: [field('status', 'Status', 'ស្ថានភាព', '', '', 'select', ACTIVE_STATUS)],
  }),
  module({
    ...
    columns: [column('code', 'Code', 'លេខកូដ'), column('name', 'Name', 'ឈ្មោះ'), column('version', 'Version', 'កំណែ'), column('description', 'Description', 'បរិយាយ'), column('attributeCount', 'Attribute Count', 'ចំនួនលក្ខណៈ'), column('status', 'Status', 'ស្ថានភាព')],
    fields: [field('code', 'Code', 'លេខកូដ', undefined, undefined, 'text', undefined, true), field('name', 'Name', 'ឈ្មោះ', undefined, undefined, 'text', undefined, true), field('description', 'Description', 'បរិយាយ', undefined, undefined, 'textarea', undefined, false, { colSpan: 2 }), field('version', 'Version', 'កំណែ', 'Version', 'កំណែ', 'text', undefined, true), field('status', 'Status', 'ស្ថានភាព', 'Control', 'ការគ្រប់គ្រង', 'select', ACTIVE_STATUS)],
    tables: [{
      key: 'attributes', title: 'Template Attributes', titleKm: 'លក្ខណៈគំរូ', addLabel: 'Add attribute',
      columns: [
        { key: 'code', label: 'Code', labelKm: 'លេខកូដ' }, { key: 'label', label: 'Label', labelKm: 'ស្លាក' }, { key: 'dataType', label: 'Data Type', labelKm: 'ប្រភេទទិន្នន័យ', type: 'select', options: ['Text', 'Number', 'Date', 'DateTime', 'Boolean', 'Reference', 'JSON'] },
        { key: 'inputType', label: 'Input Type', labelKm: 'ប្រភេទបញ្ចូល' }, { key: 'required', label: 'Required', labelKm: 'តម្រូវ', type: 'checkbox', options: YES_NO }, { key: 'repeatable', label: 'Repeatable', labelKm: 'អាចធ្វើឡើងវិញ', type: 'checkbox', options: YES_NO },
  ...
  module({
    ...
    columns: [column('tradeDirection', 'Trade Direction', 'ទិសដៅពាណិជ្ជកម្ម'), column('componentGroup', 'Component Group', 'ក្រុមសមាសភាគ'), column('componentTemplate', 'Component Template', 'គំរូសមាសភាគ'), column('templateVersion', 'Template Version', 'កំណែគំរូ'), column('required', 'Required', 'តម្រូវ'), column('repeatable', 'Repeatable', 'អាចធ្វើឡើងវិញ'), column('displayOrder', 'Display Order', 'លំដាប់បង្ហាញ'), column('status', 'Status', 'ស្ថានភាព')],
    fields: [field('tradeDirection', 'Trade Direction', 'ទិសដៅពាណិជ្ជកម្ម', undefined, undefined, 'select', DIRECTIONS, true), field('componentGroup', 'Component Group', 'ក្រុមសមាសភាគ', undefined, undefined, 'text', undefined, true), field('componentTemplate', 'Component Template', 'គំរូសមាសភាគ', undefined, undefined, 'text', undefined, true), field('templateVersion', 'Template Version', 'កំណែគំរូ', 'Version', 'កំណែ', 'text', undefined, false, { computed: true }), field('required', 'Required', 'តម្រូវ', 'Rules', 'ច្បាប់', 'checkbox', YES_NO), field('repeatable', 'Repeatable', 'អាចធ្វើឡើងវិញ', 'Rules', 'ច្បាប់', 'checkbox', YES_NO), field('displayOrder', 'Display Order', 'លំដាប់បង្ហាញ', 'Display', 'ការបង្ហាញ', 'number'), field('status', 'Status', 'ស្ថានភាព', 'Control', 'ការគ្រប់គ្រង', 'select', ACTIVE_STATUS)],
  ...
  module({
    ...
    columns: [column('accountName', 'Account Name', 'ឈ្មោះគណនី'), column('ledgerCode', 'Ledger Account', 'គណនីសៀវភៅ'), column('accountType', 'Type', 'ប្រភេទ'), column('currency', 'Currency', 'រូបិយប័ណ្ណ'), column('bankName', 'Bank', 'ធនាគារ'), column('accountNumberMasked', 'Account Number', 'លេខគណនី'), column('status', 'Status', 'ស្ថានភាព')],
  ...
  module({
    ...
    columns: [column('entryNo', 'Entry No.', 'លេខទិនានុប្បវត្តិ'), column('entryType', 'Entry Type', 'ប្រភេទទិនានុប្បវត្តិ'), column('entryDate', 'Entry Date', 'កាលបរិច្ឆេទទិនានុប្បវត្តិ'), column('postingDate', 'Posting Date', 'កាលបរិច្ឆេទចុះបញ្ជី'), column('periodName', 'Accounting Period', 'រយៈពេលគណនេយ្យ'), column('branchName', 'Branch', 'សាខា'), column('sourceDocumentNo', 'Source Document', 'ឯកសារប្រភព'), column('description', 'Description', 'បរិយាយ'), column('debitTotal', 'Total Debit', 'ឥណពន្ធសរុប'), column('creditTotal', 'Total Credit', 'ឥណទានសរុប'), column('status', 'Status', 'ស្ថានភាព'), column('createdBy', 'Created By', 'បង្កើតដោយ'), column('postedBy', 'Posted By', 'ចុះបញ្ជីដោយ'), column('postedAt', 'Posted At', 'ពេលចុះបញ្ជី')],
    ...
    tables: [{ key: 'lines', title: 'Journal Lines', titleKm: 'ជួរទិនានុប្បវត្តិ', addLabel: 'Add journal line', columns: [
      { key: 'lineNo', label: 'Line No.', labelKm: 'លេខជួរ', type: 'number' }, { key: 'account_code', label: 'Account', labelKm: 'គណនី' }, { key: 'party', label: 'Party', labelKm: 'ដៃគូ' }, { key: 'branch', label: 'Branch', labelKm: 'សាខា' }, { key: 'serviceOrder', label: 'Service Order', labelKm: 'បញ្ជាសេវាកម្ម' }, { key: 'financialDocument', label: 'Financial Document', labelKm: 'ឯកសារហិរញ្ញវត្ថុ' }, { key: 'description', label: 'Description', labelKm: 'បរិយាយ' }, { key: 'debit_amount', label: 'Debit', labelKm: 'ឥណពន្ធ', type: 'number' }, { key: 'credit_amount', label: 'Credit', labelKm: 'ឥណទាន', type: 'number' }, { key: 'currency', label: 'Currency', labelKm: 'រូបិយប័ណ្ណ', type: 'select', options: CURRENCIES }, { key: 'exchangeRate', label: 'Exchange Rate', labelKm: 'អត្រាប្តូរ', type: 'number' }, { key: 'baseDebit', label: 'Base Debit', labelKm: 'ឥណពន្ធគោល', type: 'number' }, { key: 'baseCredit', label: 'Base Credit', labelKm: 'ឥណទានគោល', type: 'number' },
    ] }],
  ...
  module({
    ...
    columns: [column('year', 'Year', 'ឆ្នាំ'), column('month', 'Month', 'ខែ'), column('startDate', 'Start Date', 'ថ្ងៃចាប់ផ្តើម'), column('endDate', 'End Date', 'ថ្ងៃបញ្ចប់'), column('status', 'Status', 'ស្ថានភាព'), column('postingCount', 'Posting Count', 'ចំនួនការចុះបញ្ជី'), column('closedBy', 'Closed By', 'បិទដោយ'), column('closedAt', 'Closed At', 'ពេលបិទ')],
